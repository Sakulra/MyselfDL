...
#生成数据集y=5+1.2x-3.4x**2/2!+5.6x**3/3!+ε
...
#power(x,y)计算x的y次方
#poly_features就是input，每一行就是一个没有除以阶乘的y的自变量组合，每一行包含20个对象，分别为x的0次方到19次方
...
print('数组labels:',labels[:2],'labels"s shape:',labels.shape)
...
def train(train_features,test_features,train_labels,test_labels,num_epochs=400):
    ...
    input_shape = train_features.shape[-1]#列数
    # 不设置偏置，因为我们已经在多项式中实现了它，就是ε。
    net = nn.Sequential(nn.Linear(input_shape,1,bias=False))#输出就是y^的值,单层线性网络
    batch_size = min(10,train_labels.shape[0])

    print(batch_size)

    #train_iter实际上是一个迭代器，类似于列表之类的，每个对象包含batch_size个元素，每处理完一个对象就会迭代下一个对象。
    train_iter = load_array((train_features,train_labels.reshape(-1,1)),batch_size)#其实不用reshepe，它就是一列
    ...
    for epoch in range(num_epochs):
        oldd2l.train_epoch_ch3(net,train_iter,loss,updater,printloss=False)
        if epoch == 0 or (epoch + 1) % 20 == 0:#只有训练轮数20的倍数才记录一次
